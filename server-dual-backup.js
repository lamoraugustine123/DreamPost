...
app.post('/api/posts', async (req, res) => {
    ...
    try {
        ...
        const sqliteQuery = 'INSERT INTO posts (id, author_email, author_name, title, text, mood, content_type, public, created_at, image_url, video_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
app.get('/api/admin/clients', checkAdminPermission('view_clients'), async (req, res) => {
    try {
        ...
        let query = 'SELECT id, name, email, role, created_at, last_login, login_count FROM users WHERE 1=1';
        const params = [];
